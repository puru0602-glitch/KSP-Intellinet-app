...
import { useLanguage, DISTRICT_NAMES_KN, CRIME_TYPE_NAMES_KN } from "@/context/language-context";
          ...
          {view === "network" && (
            <>
              <SectionHeader
                ...
                title={
                  lang === "kn"
                    ? "ಅಪರಾಧ ಜಾಲ ಮತ್ತು ಲಿಂಕ್ ವಿಶ್ಲೇಷಣೆ"
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-gradient-to-r from-amber-50 via-white to-blue-50 border border-slate-200 p-4 rounded-xl shadow-xs">
        <div>
          <div className="flex items-center gap-2 text-xs font-semibold text-amber-800 uppercase tracking-wider">
            ...
            <span>
              {lang === "kn" ? "ಸಂವಾದಾತ್ಮಕ ಡ್ಯಾಶ್‌ಬೋರ್ಡ್" : "Interactive Recharts Dashboard"}
            </span>
          </div>
          <h2 className="text-lg font-bold text-slate-900 mt-0.5">
            {lang === "kn"
              ? "ಅಪರಾಧ ಪ್ರಕಾರಗಳ ಆವರ್ತನ ಮತ್ತು ವರ್ಗೀಕರಣ ವಿಶ್ಲೇಷಣೆ"
        ...
        {onNavigate && (
          ...
          >
            <span>
              {lang === "kn" ? "ಆವರ್ತನ ಡ್ಯಾಶ್‌ಬೋರ್ಡ್ ತೆರೆಯಿರಿ" : "Open Frequency Dashboard"}
            </span>
