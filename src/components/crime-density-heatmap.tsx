...
export function CrimeDensityHeatmap(props: CrimeDensityHeatmapProps) {
  const [HeatmapComponent, setHeatmapComponent] =
    useState<ComponentType<CrimeDensityHeatmapProps> | null>(null);
  ...
  if (!HeatmapComponent) {
    return (
      <div
        className={`rounded-xl border border-slate-200 bg-white p-5 shadow-xs space-y-4 ${props.className || ""}`}
      >
